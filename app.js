const WORDS_URL = 'data/words.json?v=20260825m';
const INDEX_BASE = 'data/index/';
const MINDMAP_BASE = 'data/mindmap/';
const WORDS_BASE = 'data/words/';
const STATS_URL = 'data/stats.json?v=20260825m';
...
async function init() {
  ...
  loadStats();
  try {
    const [wr, mr] = await Promise.all([fetch(WORDS_URL + '?v=20260825m'), fetch(WORDS_BASE + 'manifest.json?v=20260825m')]);
...
async function search(rawWord) {
  ...
  try {
    ...
    const [res] = await Promise.all([fetch(WORDS_BASE + rel + '?v=20260825m'), ensureMindmap(letter), ensureFamily()]);
