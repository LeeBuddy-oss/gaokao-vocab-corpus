const WORDS_URL = 'data/words.json?v=20260826uuu';
const INDEX_BASE = 'data/index/';
const MINDMAP_BASE = 'data/mindmap/';
const WORDS_BASE = 'data/words/';
const STATS_URL = 'data/stats.json?v=20260826uuu';
const DICT_URL = 'data/en_cn_dict.json?v=20260826uuu';
...
async function init() {
  ...
  try {
    const [wr, mr, dr] = await Promise.all([
      fetch(WORDS_URL + '?v=20260826uuu'),
      fetch(WORDS_BASE + 'manifest.json?v=20260826uuu'),
      fetch(DICT_URL)
    ]);
...
async function search(rawWord) {
  ...
  try {
    ...
    const [res] = await Promise.all([fetch(WORDS_BASE + rel + '?v=20260826uuu'), ensureMindmap(letter), ensureFamily()]);
